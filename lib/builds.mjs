...
export class Builds {
    constructor() {
        this.porRepo = 15;
        this.porRepoLongo = 50;
        this.segundos = 20;
        this.janelas = [5, 15, 30, 60];
        this.etags = new Map();
    }

    janela(minutos) {
        return this.janelas.includes(Number(minutos)) ? Number(minutos) : this.janelas[0];
    }

    async resumo(projetos, minutos = 5) {
        const janela = this.janela(minutos);
        const achados = await Promise.all(projetos.map(p => this._doRepo(p, janela)));
        ...
        return {
            ...
            concluidas,
            janela,
            cortados: achados.filter(a => a.cortado).map(a => a.projeto),
            semResposta
        };
    }

    async detalhe(projetos, minutos = 5) {
        const r = await this.resumo(projetos, minutos);
        r.runs = await Promise.all(r.runs.map(async run => ({ ...run, jobs: await this._jobs(run) })));
        return r;
    }

    async _doRepo(projeto, janela) {
        const slug = await this._slug(projeto);
        if (!slug) {
            return { projeto, erro: 'sem remote origin no GitHub' };
        }
        const pagina = janela > this.janelas[0] ? this.porRepoLongo : this.porRepo;
        const dados = await this._gh(`repos/${slug}/actions/runs?per_page=${pagina}`);
        if (dados.erro) {
            return { projeto, erro: dados.erro };
        }
        const todas = dados.workflow_runs || [];
        const desde = Date.now() - janela * 60000;
        ...
        const cortado = todas.length === pagina && Date.parse(ultima?.updated_at) >= desde;
        return { projeto, runs, concluidas, cortado };
    }
