...
export function projetoDoPlist(caminho) {
    return doPlist(caminho, /<key>WorkingDirectory<\/key><string>([^<]+)<\/string>/);
}

// A porta do agente alheio, para o aviso de disputa só sair quando há disputa de verdade: avisar que
// dois agentes brigam por uma porta que eles não compartilham é o aviso que ensina a ignorar avisos.
export function portaDoPlist(caminho) {
    const p = doPlist(caminho, /<key>PORT<\/key><string>(\d+)<\/string>/);
    return p ? Number(p) : null;
}

function doPlist(caminho, padrao) {
    try {
        return (readFileSync(caminho, 'utf8').match(padrao) || [])[1] || null;
    } catch {
        return null;
    }
}
