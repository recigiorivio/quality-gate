...
export class Lint {
    ...
    detectar(projeto) {
        ...
        const linters = [];
        const temConfig = existsSync(join(raiz, 'eslint.config.mjs')) || existsSync(join(raiz, 'eslint.config.js'));
        // Config sozinha não cobre nada: sem `npm install` o `npx eslint` não roda, e a Cobertura
        // creditaria uma camada que não existe.
        if (temConfig && existsSync(join(raiz, 'node_modules/.bin/eslint'))) {
    ...
    async rodar(projeto, arquivos) {
        ...
        for (const linter of linters) {
            ...
            if (parte.falha) {
                falhas.push({ nome: linter.nome, motivo: parte.falha, arquivos: alvos.length });
                continue;
            }
            achados.push(...parte);
        }
        ...
        return {
            linters: usados, falhas,
            achados: achados.slice(0, LIMITE_ACHADOS), total: achados.length
        };
    }

    // eslint sai com código 1 quando acha erro: o erro do processo é o resultado, não uma falha.
    async _eslint(raiz, alvos) {
        const { saida, motivo } = await this._exec('npx', ['eslint', '-f', 'json', '--no-error-on-unmatched-pattern', ...alvos], raiz);
        let lista;
        try {
            lista = JSON.parse(saida.slice(saida.indexOf('[')));
        } catch {
            return { falha: motivo || 'saída do eslint não era JSON' };
        }
        const achados = [];
    ...
    async _ruff(raiz, bin, alvos) {
        const { saida, motivo } = await this._exec(bin, ['check', '--output-format=json', '--force-exclude', ...alvos], raiz);
        let lista;
        try {
            lista = JSON.parse(saida.slice(saida.indexOf('[')));
        } catch {
            return { falha: motivo || 'saída do ruff não era JSON' };
        }
        return lista.map(m => ({
            ferramenta: 'ruff',
    ...
    // Linter que sai com código != 0 porque ACHOU coisa é normal e traz o JSON no stdout. O que não
    // pode passar batido é o que nem rodou: sem stdout, "nenhum achado" seria aprovação inventada.
    async _exec(cmd, args, cwd) {
        try {
            ...
            const motivo = (`${e.stderr || ''}`.trim().split('\n')[0] || e.message || 'sem saída')
                .slice(0, 160);
            return { saida: '', motivo };
        }
    }
