...
export class Qualidade {
    ...
    esqueleto() {
        return [
            ['branch', 'Branch certa e chamado', 'chamado'],
            ['testes', 'Política de testes', 'testes'],
            ['lint', 'Lint do projeto', 'testes'],
    ...
    local(chamado, projeto, ref = '') {
        ...
        itens.push(this._item('lint', 'Lint do projeto', 'manual', 'npm run check — rodar pelo botão', [], 'testes'));
    ...
    _itensDoRepo(repo, projeto) {
        ...
        if (!repo) {
            itens.push(this._item('branch', 'Branch certa e chamado', 'atencao',
                `${projeto} não está na branch do chamado`, [], 'chamado'));
        } else {
            itens.push(this._item('branch', 'Branch certa e chamado',
                ...
                repo.sujo ? [`${repo.sujo} arquivo(s) não commitado(s)`] : [], 'chamado'));
        }
    ...
    _itensDoDiff(achados) {
        const itens = [];
        const specs = this._porRegra(achados, ['spec-novo-no-workflow-manager']);
        itens.push(this._item('testes', 'Política de testes', specs.length ? 'aviso' : 'manual',
            ...
            specs.map(a => `${a.caminho} — ${a.trecho}`)
                .concat('Reverter a correção e confirmar que o teste quebra'), 'testes'));
        return itens;
    }
